...
def horizontal_flip(oldimage):
    ...
    oldImage, but flipped about a horizontal
...
def diagonal_mirror(image):
    """ 
    image is mirrored about the diagonal
    line running from upper left to lower right.
...
def diagonal_mirror_test(imageFile):
    ...
    image_window = ImageWin("Image Processing", image.getWidth(), image.getHeight())
    image.draw(image_window)
    
    diagonal_mirror(image)   
    mirror_image_window = ImageWin("Image Processing", 
